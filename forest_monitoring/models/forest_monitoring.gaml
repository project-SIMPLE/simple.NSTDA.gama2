...
// เพิ่มต้นเก่า 10 ต้น ใช้ prefab เดิมของเกมแรก
// การเกิดหญ้า 9 ช่อง
...
// 26Aug2025
// จากรูปของพี่เติ้ลใน webex สร้างฟังก์ชั่นชื่อ "QuestionairaData" รับ 2 str โดย header รับว่าเป็น ก่อนและหลัง ส่วน Message ส่งมาเป็น str ของ ตัวเลข
// เมื่อทุกคนตอบแบบสอบถามเสร็จ ก็จะได้รับ message ว่าแต่ละคนตอบอะไร 
// เมื่อทุกคนตอบคำถามเสร็จจะขึ้นหน้า congratuation แล้วกด play ใน GAMA เพื่อไปหน้า Ready แล้วทุกคนให้กด Ready ให้ครบก่อนถึงจะกด play อีกครั้งได้
// รอบแรก กด start -> ready ได้เลย พอ ready ครับ GAMA จะกด start ได้ ถ้าไม่ครบให้กดไม่ได้ -> กด start 
...
//	float y_adaptive <- 0.5;
	...
	init{
		...
		ask tree_area[0]{
			usable_area_for_tree <- self.shape - 1;
		}
...
//		save usable_area_for_wildfire to:"../includes/export/usable_area_for_wildfire.shp" format:"shp";
		save usable_area_for_tree to:"../includes/export/usable_area_for_old_tree.shp" format:"shp";
...
////								point at_location <- {((83.33*n)+16.67+x_adaptive+(tree_distance*j)+
////									rnd(-tree_distance/rnd_location,tree_distance/rnd_location))#m,
////									((75*m)+17.5+y_adaptive+(tree_distance*i)+
////									rnd(-tree_distance/rnd_location,tree_distance/rnd_location))#m,0};
//								location <- at_location;
//								shape <- circle(size_of_tree#cm);
//								tree_type <- temp_type;
//								it_state <- 1;
